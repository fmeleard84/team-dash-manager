-- Create Kanban tables and storage policies for project-scoped Drive
...
create policy if not exists "Owners can manage kanban boards"
...
create policy if not exists "Assigned candidates can view kanban boards"
  on public.kanban_boards
  for select
  using (
    exists (
...
create policy if not exists "Owners can manage kanban columns"
...
create policy if not exists "Assigned candidates can view kanban columns"
  on public.kanban_columns
  for select
  using (
    exists (
...
create policy if not exists "Owners can manage kanban cards"
...
create policy if not exists "Assigned candidates can view kanban cards"
  on public.kanban_cards
  for select
  using (
    exists (
...
-- updated_at triggers
create trigger if not exists trg_kanban_boards_updated_at
before update on public.kanban_boards
for each row execute function public.update_updated_at_column();

create trigger if not exists trg_kanban_columns_updated_at
before update on public.kanban_columns
for each row execute function public.update_updated_at_column();

create trigger if not exists trg_kanban_cards_updated_at
before update on public.kanban_cards
for each row execute function public.update_updated_at_column();


-- 2) Storage policies for project-scoped drive
-- Allow project owners to manage files under a path prefixed by the project id (e.g., "<project_id>/...")
create policy if not exists "Owners manage project files"
  on storage.objects
  for all
  using (
