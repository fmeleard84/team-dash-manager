...
-- Policy for candidates using their email from JWT
CREATE POLICY "Candidates can manage their own sessions"
  ...
  USING (
    EXISTS (
      ...
      WHERE cp.id = time_tracking_sessions.candidate_id
      AND cp.email = COALESCE(auth.jwt() ->> 'email', '')
    )
  );

-- Policy for clients to view their project sessions
...
CREATE POLICY "Candidates can manage their active tracking"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM candidate_profiles cp
      WHERE cp.id = active_time_tracking.candidate_id
      AND cp.email = COALESCE(auth.jwt() ->> 'email', '')
    )
  );
...
CREATE POLICY "Candidates can view their own rates"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM candidate_profiles cp
      WHERE cp.id = candidate_hourly_rates.candidate_id
      AND cp.email = COALESCE(auth.jwt() ->> 'email', '')
    )
  );
