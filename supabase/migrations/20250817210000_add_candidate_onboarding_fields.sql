-- Migration: Ajouter les champs d'onboarding candidat
-- Date: 2025-08-17

-- Ajouter les nouveaux champs pour l'onboarding candidat
...
-- Index pour optimiser les requêtes sur les résultats de tests
...
-- Trigger pour mettre à jour automatiquement updated_at sur candidate_qualification_results
DROP TRIGGER IF EXISTS update_candidate_qualification_results_updated_at ON candidate_qualification_results;
CREATE TRIGGER update_candidate_qualification_results_updated_at
    BEFORE UPDATE ON candidate_qualification_results
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- RLS pour candidate_qualification_results
ALTER TABLE candidate_qualification_results ENABLE ROW LEVEL SECURITY;

-- Policy pour que les candidats puissent voir/modifier leurs propres résultats
...
BEGIN
    ...
    AND EXISTS (
        -- Vérifier que le candidat a au moins une expertise correspondante
        SELECT 1 FROM hr_resource_assignments hra
        JOIN candidate_expertises ce ON ce.expertise_id = ANY(candidate_expertise_ids)
        WHERE hra.project_id = p.id
    )
