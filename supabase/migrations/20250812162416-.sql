-- Retry migration with safe policy creation using DO blocks
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_update_project_events_updated_at') THEN
    CREATE TRIGGER trg_update_project_events_updated_at
    BEFORE UPDATE ON public.project_events
    FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_update_project_event_attendees_updated_at') THEN
...
-- Policies for project_events
DO $$ BEGIN
  CREATE POLICY "Project owners can view their events"
...
DO $$ BEGIN
  CREATE POLICY "Project owners can create events"
  ON public.project_events
  FOR INSERT
  WITH CHECK (
    EXISTS (
...
DO $$ BEGIN
  CREATE POLICY "Project owners can update their events"
  ON public.project_events
  FOR UPDATE
  USING (
    EXISTS (
...
DO $$ BEGIN
  CREATE POLICY "Project owners can delete their events"
  ON public.project_events
  FOR DELETE
  USING (
    EXISTS (
...
DO $$ BEGIN
  CREATE POLICY "Attendees can view their invitations"
  ON public.project_event_attendees
  FOR SELECT
  USING (
    profile_id = auth.uid()
...
DO $$ BEGIN
  CREATE POLICY "Owners can view project files"
  ON storage.objects
  FOR SELECT
  USING (
...
DO $$ BEGIN
  CREATE POLICY "Owners can update project files"
  ON storage.objects
  FOR UPDATE
  USING (
    bucket_id = 'project-files'
