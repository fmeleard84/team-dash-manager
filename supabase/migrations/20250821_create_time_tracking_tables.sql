...
CREATE TABLE IF NOT EXISTS rate_modifiers (
  ...
  reference_id UUID, -- Can reference hr_languages or hr_expertises
...
CREATE INDEX idx_time_tracking_sessions_candidate ON time_tracking_sessions(candidate_id);
CREATE INDEX idx_time_tracking_sessions_project ON time_tracking_sessions(project_id);
CREATE INDEX idx_time_tracking_sessions_status ON time_tracking_sessions(status);
CREATE INDEX idx_active_time_tracking_project ON active_time_tracking(project_id);
CREATE INDEX idx_active_time_tracking_status ON active_time_tracking(status);
...
CREATE POLICY "Candidates can manage their own sessions"
  ...
  USING (
    candidate_id IN (
      SELECT id FROM candidate_profiles WHERE email = auth.jwt() ->> 'email'
    )
  );
...
-- RLS Policies for active_time_tracking
...
-- RLS Policies for candidate_hourly_rates
...
-- RLS Policies for rate_modifiers
