...
-- ========================================
-- 3. GÉRER LA TABLE notifications
-- ========================================

-- Vérifier si la table notifications existe et quelles colonnes elle a
DO $$
BEGIN
    ...
    ELSE
        -- Si la table existe, vérifier si elle a la colonne is_read
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                      WHERE table_name = 'notifications' AND column_name = 'is_read') THEN
            -- Vérifier si elle a la colonne read (ancien nom)
...
BEGIN
    ...
    -- Vérifier notification is_read
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'notifications' AND column_name = 'is_read'
    ) INTO v_exists;
    RAISE NOTICE '✅ Colonne is_read dans notifications: %', v_exists;
    
    RAISE NOTICE '';
    RAISE NOTICE '🎉 Script exécuté avec succès!';
    RAISE NOTICE '📝 N''oubliez pas de mettre à jour manuellement les téléphones si nécessaire.';
END $$;