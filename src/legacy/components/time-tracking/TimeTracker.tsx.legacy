...
export const TimeTracker = () => {
  ...
  // Debug: log all projects to see their status
  console.log('All projects in TimeTracker:', projects);
  
  // Get active projects - only check project status is 'play'
  const activeProjects = projects.filter(p => {
    console.log('Project:', p.project_title, 'Status:', p.project_status, 'Booking:', p.booking_status);
    // Un projet est actif s'il est en status 'play'
    return p.project_status === 'play';
  });
  ...
  const handleStart = async () => {
    ...
    const project = activeProjects.find(p => p.project_id === selectedProjectId);
    if (!project) return;
          ...
          {activeProjects.length === 0 ? (
            <div className="text-center py-6 text-muted-foreground">
              ...
              <p className="text-xs mt-1">
                Vous devez avoir un projet en cours pour démarrer le chronomètre
              </p>
            </div>
          ) : (
            <>
